...
def main(args: Args):
    ...
    nf_models = load_nf_models(args.models_dir, device="cpu")
    if len(nf_models) != 8:
        raise ValueError(
            f"Expected to load exactly 8 models (4 for 1-jet and 4 for 2-jet) but found {len(nf_models)}."
        )
    ...
    j2_data, j2_detlabel, _, _ = createMultiJetMultiNuanData(
        2, False, seed=0, root_dir=args.root_dir
    )
    j1_data, j1_detlabel, _, _ = createMultiJetMultiNuanData(
        1, False, seed=0, root_dir=args.root_dir
    )
    j2_data = j2_data.cpu()
    ...
    with torch.no_grad():
        ...
        j1_data = torch.cat(
            [j1_data, NF_feat_s1j, NF_feat_s1j_3, NF_feat_b1j, NF_feat_b1j_3], dim=1
        )
        j2_data = torch.cat(
            [j2_data, NF_feat_s2j, NF_feat_s2j_3, NF_feat_b2j, NF_feat_b2j_3], dim=1
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train CombinedClassifier with appended Normalizing Flow features"
    )
